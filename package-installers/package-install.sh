...
echo "basic bundle [1] (git python3 vim curl wget htop net-tools)\n 
...
case "$choice" in 
    1) sudo apt install -y git python3 vim curl wget htop net-tools
    ;;
    ...
    ;;
    3) sudo apt install -y git python3 vim curl wget htop net-tools nmap net-tools aircrack-ng wireshark ettercap-text-only john hydra nikto sqlmap 
    curl https://raw.githubusercontent.com/rapid7/metasploit-omnibus/master/config/templates/metasploit-framework-wrappers/msfupdate.erb > msfinstall && \chmod 755 msfinstall && \./msfinstall
    ;;
    *) echo "Invalid choice" 
    ;;
